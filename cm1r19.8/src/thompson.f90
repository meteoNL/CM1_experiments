...
!.. fill in just a basic vertical profile, somewhat boundary-layer following.
      ...
      if (max_test .lt. eps) then
         ...
         do i = its, min(ide-1,ite)
            ...
            nwfa2d(i,j) = nwfa(i,j,1) * 0.000196 * (airmass*2.E-10)
            do k = 2, kte
...
!$omp rho,prv_rev,prw_vcd,tem2,dbz,nc1d,nwfa1d,nifa1d,nwfa1)
...
!-GT        nwfa1d(kts) = nwfa1
         ...
         IF (has_reqc.ne.0 .and. has_reqi.ne.0 .and. has_reqs.ne.0) THEN
          ...
          do k = kts, kte
             re_cloud(i,j,k) = MAX(2.49E-6, MIN(re_qc1d(k), 50.E-6))
             re_ice(i,j,k)   = MAX(4.99E-6, MIN(re_qi1d(k), 125.E-6))
             re_snow(i,j,k)  = MAX(9.99E-6, MIN(re_qs1d(k), 999.E-6))
          enddo
         ENDIF
...
! Need to split loops between 1 processes to speedup
! (2017Jul26, Jason Do)
        km_s = 0
        km_e = ntb_IN*45 - 1
