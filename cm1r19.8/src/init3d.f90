...
!  Set winds to base-state values:
      ...
      IF( sgsmodel.eq.1 .and. ( testcase.eq.3 .or. testcase.eq.4 .or.  testcase.eq.5 ) )THEN
...
!!!        yref  =  -100000.0    ! y location of TC center (m)
        ...
        ! for all tctypes:
        zdd         =    20000.0     ! height (ASL) where V=0
...
! Random perturbations:
      ...
      ELSEIF( testcase.eq.1 .or. testcase.eq.2 .or. testcase.eq.6 )THEN
...
!  cm1r18:  get total mass of dry air at t=0
      ...
      IF( .not. restarted )THEN
        ...
        call MPI_ALLREDUCE(mpi_in_place,mass1,1,MPI_DOUBLE_PRECISION,MPI_SUM,MPI_COMM_WORLD,ierr)

        mass1 = mass1*dble(dx*dy*dz)
