  ...
  private
  public :: statpack,setup_stat_vars,write_statsctl

  CONTAINS

      subroutine statpack(nrec,ndt,dt,dtlast,rtime,adt,acfl,cloudvar,         &
                          ...
                          ua,va,wa,ppi,tha,qa,vq  ,kmh,kmv,khh,khv,tkea,      &
                          xkzh,xkzq,xkzm,                                     &
...
121       format(a3)
        ...
        if(stat_tke.eq.1) call maxmin(ni,nj,nk+1,tkea,nstat,rstat,kmin,kmax,'TKEMAX','TKEMIN')
...
!-----------------------------------------------------------------------
      ...
      if( stat_out.gt.0 .and. output_format.eq.1 )then
        ! write GrADS descriptor file:
        call write_statsctl(name_stat,desc_stat,unit_stat,  &
                            tdef,1+nint(timax/max(statfrq,dtl)))
      endif
...
!ccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
      ...
      subroutine write_statsctl(name_stat,desc_stat,unit_stat,  &
                                tdef,numt)
      use input
...
156   format(a3)
301   format('dset ^',a70)
302   format('title cm1r19 output, stats')
