...
! u vars:
    ...
    IF( output_ubudget.eq.1 )THEN
      ...
      if( ud_diag.ge.1 )then
      n_out = n_out + 1
      name_output(n_out) = 'ub_diag'
      desc_output(n_out) = 'u budget: our new vertical advection diag.'
      unit_output(n_out) = 'm/s/s'
      grid_output(n_out) = 'u'     ! s=scalar pts (3d) ; u=u pts (3d) ; v=v pts (3d) ; w=w pts (3d) ; 2=2d scalar pts
      endif
...
! v vars:
    ...
    IF( output_vbudget.eq.1 )THEN
      ...
      if( vd_diag.ge.1 )then
      n_out = n_out + 1
      name_output(n_out) = 'vb_diag'
      desc_output(n_out) = 'v budget: our new vertical advection diag.'
      unit_output(n_out) = 'm/s/s'
      grid_output(n_out) = 'v'     ! s=scalar pts (3d) ; u=u pts (3d) ; v=v pts (3d) ; w=w pts (3d) ; 2=2d scalar pts
      endif
...
!ccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
      ...
      subroutine writeout(srec,urec,vrec,wrec,rtime,dt,fnum,nwrite,qname,                      &
                        ...
                        num_soil_layers,u10,v10,s10,t2,q2,znt,ust,tst,qst,z0t,z0q,u1,v1,s1,    &
      ...
      real, dimension(ib:ie,jb:je), intent(in) :: znt,ust,tst,qst,z0t,z0q,u1,v1,s1
...
!!!        if( myid.eq.0 ) print *,'    2d vars '
      ...
      !c-c-c-c-c-c-c-c-c-c-c-c-c-c-c-c-c-c-c-c-c-c-c-c-c-c-c!
        ...
        elseif( trim(name_output(n)).eq.'z0t' )then
        ...
        elseif( trim(name_output(n)).eq.'z0q' )then
...
!!!        if( myid.eq.0 ) print *,'    u vars '
        ...
        elseif( trim(name_output(n)).eq.'ub_diag' )then
          ...
          do j=1,nj
          do i=1,ni+1
            dumu(i,j,k) = udiag(i,j,k,ud_diag)
          enddo
          enddo
          enddo
...
!!!        if( myid.eq.0 ) print *,'    v vars '
        ...
        elseif( trim(name_output(n)).eq.'vb_diag' )then
          ...
          do j=1,nj+1
          do i=1,ni
            dumv(i,j,k) = vdiag(i,j,k,vd_diag)
          enddo
          enddo
          enddo
