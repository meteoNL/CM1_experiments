...
! prepocessed on "Apr  2 2018" at "10:37:29"
...
!                    1 = mass-weighted fall speed; 2 = number-weighted fallspeed.
  ...
  real, public    :: icefallfac = 1.0 ! factor to adjust rain fall speed
  real, public    :: snowfallfac = 1.0 ! factor to adjust rain fall speed
...
!  integer, private :: infdo = 1   ! 1 = calculate number-weighted fall speeds
  ...
  integer, public :: irimdenopt = 1 ! = 1 for default Maklin; = 2 for experimental Cober and List (1993)
     ...
     &          ,an,dn1 )
        ...
        DO kz = kts,kte
          DO ix = its,ite
             ...
             IF ( .not. present(qi) ) re_snow(ix,jy,kz)  = MAX(10.E-6, MIN(t3(ix,1,kz), 125.E-6))
          ENDDO
         ENDDO
...
!           nwfa(ix,jy,kz) = Min(1.5e9, nwfa(ix,jy,kz) + nwfa2d(ix,jy)*dtp)
           nwfa(ix,jy,kz) = nwfa(ix,jy,kz) + nwfa2d(ix,jy)*dtp
...
!  190325:  do not update state in dbz calc.
