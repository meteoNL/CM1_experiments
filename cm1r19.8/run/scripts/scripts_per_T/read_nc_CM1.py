...
if lvl == "vert":
    viewdbz=test[varname][timing,:,:]
else: 
    viewdbz=test[varname][timing,lvl,:,:]
...
for i in np.arange(len(timing)):
    ...
    if lvl != "vert":
        pl.title(namesim+" | time = %.3d"% time[timing[i]]+" min"+ " | level = "+str(lvl)+"/"+str(maxlvl))
    else:
        pl.title(namesim+" | time = %.3d"% time[timing[i]]+" min")
    ...
    fn = str(path+namesim+"/"+name_figs+"%.3d" % timing[i]+".png")
    pl.savefig(fn)
    fnames+=[fn]
#%%
##import imageio
