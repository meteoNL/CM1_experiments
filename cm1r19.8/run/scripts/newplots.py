...
## list of simulation folders to be analysed!! 
...
## list of full names which are more readable 
...
## define some color sequence for the plot
...
# define sequence of line styles
list_styles = ["-","-","-","-","-","-","-","-","-","-","--",":","-","-","-","-","--","--","-","-","-","--","--","-","--","-",":"]
...
def genplot():
    ## generates a landscape shaped plot with four column subplots
...
def plotvariables(simarray,simname,cr="red",s="-",w="4"):
   ## needs numbers (simarray), simulation name (simname)

    ## plots the four quantities.... with the line properties as arguments cr, s, w
...
def addlegends(axis,string):
    ## adds given string as xlabel to the given axis
...
#make array for plotted numbers
...
for i in np.arange(len(list_of_sims)):
  ...
  ##  all_numbers[i,:,:nlev]=result\
    ## plot it all with the function that does it, above
...
## set plot limits!
ax1.set_xlim(-10e-5,25e-5)
ax2.set_xlim(-1e-8,3.2e-7)
ax3.set_xlim(-0.0008,0.009)
ax4.set_xlim(-2500,3000)
